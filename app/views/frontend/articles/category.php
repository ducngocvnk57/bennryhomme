...
<section class="ben-content">
	...
	<section class="right-content">
		<?php if(isset($list) && count($list)) { foreach($list as $key => $val) { ?>
			...
			window.location = '<?php echo helper_string_alias($category['title']).'/'.helper_string_alias($val['title']).'-a'.$val['id'].CMS_SUFFIX;?>';
			</script>
			<?php } ?>
			<article class="item-news">
				...
				<h2 class="title"><a href="<?php echo helper_string_alias($category['title']).'/'.helper_string_alias($val['title']).'-a'.$val['id'].CMS_SUFFIX;?>" title="<?php echo htmlspecialchars($val['title']);?>"><?php echo $val['title'];?></a></h2>
