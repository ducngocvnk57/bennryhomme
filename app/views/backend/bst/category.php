<div id="cms-tab">
	...
	<ul class="main">
		<li class="main"><a href="<?php echo CMS_BACKEND.'/bst/category'.CMS_SUFFIX;?>" class="main main-select">Quản lý danh mục bộ sưu tập</a></li>
...
<div id="cms-container">
	<div id="cms-filter">
		<div class="right">
			<input type="submit" class="button cms-publish-ajax" value="Xuất bản" name="bst_category" />
			<input type="submit" class="button cms-unpublish-ajax" value="Dừng xuất bản" name="bst_category" />
			<input type="submit" class="button cms-order-ajax" value="Sắp xếp" name="bst_category" />
			<?php if($this->auth['group'] == 'Người quản lý'){ ?>
			<input type="submit" class="button cms-delete-category-ajax" value="Xóa nhiều" name="bst_category" />
			<?php } ?>
		</div><!-- .right -->
		<div class="cms-clear"></div>
	</div><!-- #cms-filter -->
	<div id="cms-table">
		...
		<table cellspacing="0" cellpadding="0" class="data">
			...
			<tr <?php echo (($key + 1) == count($full_data))?'class="last"':'';?>>
				...
				<td class="left title"><a href="<?php echo CMS_URL.CMS_BACKEND.'/bst/editcategory/'.$val['id'].CMS_SUFFIX;?>"><?php echo str_repeat('|----- ', $val['level']).htmlspecialchars($val['title']);?></a></td>
				<td class="center"><?php echo helper_module_count_item('bst_item', $val['id']);?></td>
				...
				<td class="center">
					<a class="cms-set-ajax" href="<?php echo CMS_URL.CMS_BACKEND.'/common/set/bst_category/publish/'.$val['id'].CMS_SUFFIX;?>">
				...
				<td class="center">
					<a class="cms-set-ajax" href="<?php echo CMS_URL.CMS_BACKEND.'/common/set/bst_category/special/'.$val['id'].CMS_SUFFIX;?>">
				...
				<td class="center">
					<a href="<?php echo CMS_URL.CMS_BACKEND.'/bst/editcategory/'.$val['id'].CMS_SUFFIX;?>"><img src="template/backend/images/edit.png" /></a>
				</td>
				<td class="center last"><?php echo $val['id'];?></td>
			</tr>
			<?php } } else{ ?>
			<tr class="last"><td class="center first" colspan="14">Không có dữ liệu danh mục bộ sưu tập.</td></tr>
			<?php } ?>
		</table>
		</form>
	</div><!-- #cms-table -->
