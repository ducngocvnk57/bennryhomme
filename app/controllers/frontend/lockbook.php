...
class Lockbook extends MY_Controller{
	...
	public function category($parentid = 0, $page = 1){
		...
		$keyword = NULL;
		// print_r($keyword);die;
		$category = $this->db->from('lockbook_category')->where(array('id' => $parentid, 'publish' => 1))->get()->row_array();
		if(!isset($category) || count($category) == 0) die($this->lib_common->php_redirect(''));
		$config['use_page_numbers'] = TRUE;
			$children = $this->lib_nestedset->children('lockbook_category', array('lft >=' => $category['lft'], 'rgt <=' => $category['rgt']));
			if(!empty($keyword)) {
				$config['total_rows'] = $this->db->from('lockbook_item')->where(array('publish' => 1, 'our_price >=' => $keyword[0] , 'our_price <=' => $keyword[1]))->where_in('parentid', $children)->count_all_results();
			}else{
				$config['total_rows'] = $this->db->from('lockbook_item')->where(array('publish' => 1))->where_in('parentid', $children)->count_all_results();
			}
		// print_r($config['total_rows']);die;
		if($category['level'] == 2) {
			$cateid = helper_module_get_item_info('lockbook_category', $category['parentid']);
		...
		$config['base_url'] = $alias.'-ctl'.$category['id'].'-';
		$config['per_page'] = 15;
		...
		if($total > 0){
			...
			if($category['rgt'] - $category['lft'] > 1){
				$data['data']['list'] = $this->db->from('lockbook_item')->where(array('publish' => 1))->where_in('parentid', $children)->limit($config['per_page'], $page * $config['per_page'])->order_by('order desc, id asc')->get()->result_array();
			}
			else{
				if(!empty($keyword)) {
					$data['data']['list'] = $this->db->from('lockbook_item')->where(array('parentid' => $parentid, 'publish' => 1, 'our_price >=' => $keyword[0] , 'our_price <=' => $keyword[1]))->limit($config['per_page'], $page * $config['per_page'])->order_by('order asc, id desc')->get()->result_array();
				}else{
					$data['data']['list'] = $this->db->from('lockbook_item')->where(array('parentid' => $parentid, 'publish' => 1))->limit($config['per_page'], $page * $config['per_page'])->order_by('order desc, id desc')->get()->result_array();
				}
			}
		...
		if(!empty($keyword)) {
			$data['data']['canonical'] = CMS_URL.(($page == 0)?$alias.'-ctl'.$category['id']:$alias.'-ctl'.$category['id'].'-tl'.($page+1)).CMS_SUFFIX.'?keyword='.$keyword_i;
			$data['data']['rel_prev'] = ($page > 0)?CMS_URL.$alias.'-ctl'.$category['id'].'-tl'.($page).CMS_SUFFIX.'?keyword='.$keyword_i:'';
			$data['data']['rel_next'] = ($page < ($total - 1))?CMS_URL.$alias.'-ctl'.$category['id'].'-tl'.($page+2).CMS_SUFFIX.'?keyword='.$keyword_i:'';
		}else{
			$data['data']['canonical'] = CMS_URL.(($page == 0)?$alias.'-ctl'.$category['id']:$alias.'-ctl'.$category['id'].'-tl'.($page+1)).CMS_SUFFIX;
			$data['data']['rel_prev'] = ($page > 0)?CMS_URL.$alias.'-ctl'.$category['id'].'-tl'.($page).CMS_SUFFIX:'';
			$data['data']['rel_next'] = ($page < ($total - 1))?CMS_URL.$alias.'-ctl'.$category['id'].'-tl'.($page+2).CMS_SUFFIX:'';
		}		
		
		$data['data']['children'] = ($category['rgt'] - $category['lft'] > 1)?$this->db->select('id, title')->from('lockbook_category')->where(array('parentid' => $parentid, 'publish' => 1))->get()->result_array():NULL;
		$data['template'] = 'frontend/lockbook/category';
	...
	public function thuonghieu($labelid = 0, $page = 1){
		...
		$config['use_page_numbers'] = TRUE;
			...
			if(!empty($keyword)) {
				$config['total_rows'] = $this->db->from('lockbook_item')->where(array('publish' => 1, 'labelid' => $labelid, 'our_price >=' => $keyword[0] , 'our_price <=' => $keyword[1]))->count_all_results();
			}else{
				$config['total_rows'] = $this->db->from('lockbook_item')->where(array('publish' => 1, 'labelid' => $labelid))->count_all_results();
			}
		...
		if($total > 0){
			...
			$data['data']['list'] = $this->db->from('lockbook_item')->where(array('publish' => 1, 'labelid' => $labelid))->limit($config['per_page'], $page * $config['per_page'])->order_by('id asc')->get()->result_array();
		...
		if(!empty($keyword)) {
			$data['data']['canonical'] = CMS_URL.(($page == 0)?$alias.'-th'.$category['id']:$alias.'-ctl'.$category['id'].'-tl'.($page+1)).CMS_SUFFIX.'?keyword='.$keyword_i;
			$data['data']['rel_prev'] = ($page > 0)?CMS_URL.$alias.'-th'.$category['id'].'-tl'.($page).CMS_SUFFIX.'?keyword='.$keyword_i:'';
			$data['data']['rel_next'] = ($page < ($total - 1))?CMS_URL.$alias.'-th'.$category['id'].'-tl'.($page+2).CMS_SUFFIX.'?keyword='.$keyword_i:'';
		}else{
			$data['data']['canonical'] = CMS_URL.(($page == 0)?$alias.'-th'.$category['id']:$alias.'-th'.$category['id'].'-tl'.($page+1)).CMS_SUFFIX;
			$data['data']['rel_prev'] = ($page > 0)?CMS_URL.$alias.'-th'.$category['id'].'-tl'.($page).CMS_SUFFIX:'';
			$data['data']['rel_next'] = ($page < ($total - 1))?CMS_URL.$alias.'-th'.$category['id'].'-tl'.($page+2).CMS_SUFFIX:'';
		}		
		
		$data['template'] = 'frontend/lockbook/thuonghieu';
		$this->load->view('frontend/layouts/detailbst', $data);
	}

	public function category_order($page = 0){
		// print_r('a');die;
		$item = $this->db->from('lockbook_item')->where(array('publish' => 1))->order_by('id', 'desc')->limit(30)->get()->result_array();
		...
		$data['template'] = 'frontend/lockbook/category_order';
		$this->load->view('frontend/layouts/detailbst', $data);
	}

	public function category_sale($page = 0){
		// print_r('a');die;
		$item = $this->db->from('lockbook_item')->where(array('retail_price !=' => ""))->order_by('id', 'desc')->limit(30)->get()->result_array();
		...
		$data['template'] = 'frontend/lockbook/category_sale';
		$this->load->view('frontend/layouts/detailbst', $data);
	}

	public function item($id = 0){
		$id = (int)$id;
		$item = $this->db->from('lockbook_item')->where(array('id' => $id, 'publish' => 1))->get()->row_array();
		$data['parentid'] = $item['parentid'];
		if(!isset($item) || count($item) == 0) die($this->lib_common->php_redirect(''));
		$category = $this->db->from('lockbook_category')->where(array('id' => $item['parentid'], 'publish' => 1))->get()->row_array();
		if(!isset($category) || count($category) == 0) die($this->lib_common->php_redirect(''));
		...
		$view_lockbook_item = $this->session->userdata('view_lockbook_item_'.$item['id']);
		...
		$alias_2 = $this->lib_string->alias($category['title']);
		if(!isset($view_lockbook_item) || empty($view_lockbook_item)){
			$this->session->set_userdata('view_lockbook_item_'.$item['id'], 'ok');
			$rand = rand(0, 5);
			$value = rand(3, 5);
			if($rand == 5){
				$this->db->set('rate_value', 'rate_value + '.$value, FALSE)->set('rate_total', 'rate_total + 1', FALSE)->where(array('id' => $id))->update('lockbook_item');
				$item['rate_value'] = $item['rate_value'] + $value;
				$item['rate_total'] = $item['rate_total'] + 1;
			}
			$this->db->set('viewed', 'viewed + 1', FALSE)->where(array('id' => $item['id']))->update('lockbook_item');
			$item['viewed']++;
		}
		if(!empty($item['tags'])){
			...
			if(isset($tags) && count($tags)){
				...
				$query_sql = 'SELECT * FROM '.CMS_PREFIX.'lockbook_item WHERE ('.$field.') AND `id` != '.$item['id'].' LIMIT 0, 10';
		...
		$data['data']['canonical'] = CMS_URL.$alias.'-tl'.$item['id'].CMS_SUFFIX;
		...
		$data['data']['children'] = ($category['rgt'] - $category['lft'] > 1)?$this->db->select('id, title')->from('lockbook_category')->where(array('parentid' => $category['id'], 'publish' => 1))->get()->result_array():NULL;
		$data['data']['same'] = $this->db->select('*')->from('lockbook_item')->where(array('id !=' => $item['id'], 'parentid' => $category['id'], 'publish' => 1))->limit(10, 0)->order_by('id desc')->get()->result_array();
		$data['data']['user'] = $user;
		$data['data']['google_authorship'] = $user['author'];
		$data['template'] = 'frontend/lockbook/item';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	...
	public function search(){
		...
		$config['use_page_numbers'] = TRUE;
		$sql = 'SELECT `id` FROM `'.CMS_PREFIX.'lockbook_item` WHERE `publish` = 1 AND (`title` LIKE ? OR `description` LIKE ? OR `content` LIKE ?)';
		...
		if($config['total_rows'] > 0){
			$page = $page - 1;
			$this->pagination->initialize($config);
			$sql = 'SELECT * FROM `'.CMS_PREFIX.'lockbook_item` WHERE `publish` = 1 AND (`title` LIKE ? OR `description` LIKE ? OR `content` LIKE ?) ORDER BY `id` DESC LIMIT '.$page * $config['per_page'].', '.$config['per_page'].'';
		...
		$data['data']['total_rows'] = $config['total_rows'];
		$data['data']['menu_active'] = array('module' => 'lockbook', 'id' => 2);
		// Seo
		$data['title'] = $q;
		$data['keywords'] = $q;
		$data['description'] = $q;
		...
		// Giao diện
		$data['template'] = 'frontend/lockbook/search';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	...
	public function search_high(){
		...
		$title = helper_module_get_category_info('lockbook_category', $parentid);
		// print_r($title);die;
		
		if($order_by == 1) {
			$order_by = 'id asc';
			$list = helper_module_list_item('lockbook_item','id, title, image, price, retail_price, our_price',array('parentid' => $parentid), $order_by , 100, TRUE);
			// Seo
		...
		// Giao diện
		$data['template'] = 'frontend/lockbook/search-high';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	...
	public function tags_detail($page = 1){
		...
		$data['template'] = 'frontend/lockbook/tags_detail';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	...
	public function tags($alias = '', $page = 1){
		...
		$config['use_page_numbers'] = TRUE;
		$query_sql = 'SELECT * FROM '.CMS_PREFIX.'lockbook_item WHERE (`tags` LIKE ?)';
		...
		if($total > 0){
			$page = $page - 1;
			$this->pagination->initialize($config);
			$query_sql = 'SELECT * FROM '.CMS_PREFIX.'lockbook_item WHERE (`tags` LIKE ?) AND `publish` = 1 ORDER BY `id` DESC LIMIT '.($page * $config['per_page']).', '.$config['per_page'];
			$query_param = array('%'.$tag['title'].'%');
		...
		$data['data']['canonical'] = CMS_URL.(($page == 0)?'tags/'.$alias:'tags/'.$alias.'-tl'.($page+1)).CMS_SUFFIX;
		$data['data']['rel_prev'] = ($page > 0)?CMS_URL.'tags/'.$alias.'-tl'.($page).CMS_SUFFIX:'';
		$data['data']['rel_next'] = ($page < ($total - 1))?CMS_URL.'tags/'.$alias.'-tl'.($page+2).CMS_SUFFIX:'';
		$data['template'] = 'frontend/lockbook/tags';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	...
	public function addtocart($itemid = NULL){
		$itemid = (int)$itemid;
		$lockbook_item = $this->db->from('lockbook_item')->where(array('id' => $itemid))->get()->row_array();
		$size_van = $this->input->post('data');
		
		$size_van = $size_van?$size_van:0; 
		if(!isset($lockbook_item)){
		...
		if(!empty($lockbook_item['price'])) {
			$item = array(
				'id' => $lockbook_item['id'],
				'image' => $lockbook_item['image'],
				'title' => $lockbook_item['title'],
				'price' => $lockbook_item['price'],
				'number' => 1,
				'size' => $size_van['size'],
			...
			if(isset($cart)){
				foreach($cart as $key => $val){	
					if(($val['id'] == $item['id']) && ($val['size'] == $item['size']) && ($val['color'] == $item['color'])){
						...
						setcookie('itq_cookie_cart_'.CMS_CODE, json_encode($cart), time()+(7*24*3600), '/');
						die($this->lib_common->js_redirect('gio-hang.html'));
						// die($this->lib_common->js_redirect('gio-hang.html' , 'Cập nhật số lượng sản phẩm '.$lockbook_item['title'].' thành công. Số lượng hiện tại là: '.$cart[$key]['number']));
					}
				}
				$cart[] = $item;
				setcookie('itq_cookie_cart_'.CMS_CODE, json_encode($cart), time()+(7*24*3600), '/');
				die($this->lib_common->js_redirect('gio-hang.html'));
				// die($this->lib_common->js_redirect('gio-hang.html' , 'Thêm sản phẩm '.$lockbook_item['title'].' thành công.'));
			}
		}
	}
	...
	public function cart(){
		...
		if(isset($_COOKIE['itq_cookie_cart_'.CMS_CODE])){
			...
			if($this->input->post('btnNumber')){
				...
				if(isset($number) && isset($cart)){
					...
					header('Location: '.CMS_URL.'frontend/lockbook/cart'.CMS_SUFFIX);
					die;
				}
			}
		...
		// Giao diện
		$data['template'] = 'frontend/lockbook/cart';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
	public function payment(){
		...
		$data['template'] = 'frontend/lockbook/payment';
		$this->load->view('frontend/layouts/detailbst', $data);
	}
